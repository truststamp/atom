...
assert('constructor args treated as set(), single property', atom('a', 'b').get('a') === 'b');

assert('constructor args treated as set(), multiple properties', atom({ a: 'b', c: 'd' }).get(['a', 'c']) + '' === 'b,d');
...
a.off(aListener);
a.set('a', 'A3');
assert('off() prevents the function from being called again', results + '' === 'A1,B1,C,A2');
...
assert('off() will be selective about unbinding a listener, if keyOrList provided', results + '' === 'BB');

results = [];
a.once('c', function (c) {
	results.push(c);
});
assert('once() works when called with one condition that is already complete', results + '' === 'C');

results = [];
a.once(['b', 'a'], function (b, a) {
	results = results.concat([b, a]);
});
assert('once() works when called with two conditions that are already ' + 'complete', results + '' === 'B1,A3');
...
assert('once() works when the conditions are set with no value provided', results + '' === 'set,once,');
...
results.push('set');
a.set('f', 'F');
assert('once() works when called with one condition that is not complete', results + '' === 'set,once,F');
...
results.push('set');
a.set({ g: 'G', h: 'H' });
assert('once() works when called with multiple conditions that are ' + 'not complete', results + '' === 'set,once,G,H');
...
results.push('set');
a.set('i', 'I');
assert('once() works when called with a mix of complete and incomplete ' + 'conditions', results + '' === 'set,once,H,I');
...
a.set('a', 'A3');
assert('once() gets called only once, even when conditions are completed ' + 'multiple times', aCalls === 1);
...
a.set('a', 'A4');
assert('next() works when called with a single condition', results + '' === 'A4');
...
results.push('set');
a.set('c', 'C1');
assert('next() works when called with multiple conditions', results + '' === 'set,B1,C1,next,B1,C1');
...
results.push('setX1');
a.set('x1', 'y1');
assert('set() takes it in stride when the listener list is synchronously ' + 'modified by one of the listeners', results + '' === 'setX,x=y,setX1,x1=y1');
...
assert('need() can be called with no callback, to invoke the provider', results + '' === 'need,provider');

results = ['need'];
a.need('d', function (d) {
	results = results.concat(['satisfy', d]);
});
assert('need() calls back immediately when the (single) need is pre-satisfied', results + '' === 'need,satisfy,D');

results = ['need'];
a.need(['e', 'f'], function (e, f) {
	results = results.concat(['satisfy', e, f]);
});
assert('need() calls back immediately when the needs (plural) are pre-satisfied', results + '' === 'need,satisfy,E,F');
...
a.set('j', 'J');
a.set('j', 'J1');
assert('need() callback gets triggered after the needed value is set()', results + '' === 'need,set,satisfy,J');
...
assert('need() registered before provide() works', results + '' === 'need,provide,fulfill,satisfy,K');
...
assert('need() registered after provide() works', results + '' === 'provide,need,fulfill,satisfy,L');
...
a.chain(function (nextLink, lastArg1, lastArg2) {
...
results.push('set');
a.set({ m: 'M1', n: 'N1' });
assert('entangle() works for maps passed to set()', results + '' === 'set,once,M1,N1');
...
var mc = atom();
assert('method chaining works', mc.need('a', function (a) {
	mc.set('b', 'c');
}).next('b', function (b) {
	mc.set('c', 'd');
}).on('c', function (c) {
	...
	mc.set('e', 'f');
}).provide('a', function (done) {
	done('b');
}).chain(function () {
	mc.set('f', 'g');
}).set('success', mc.get('a,b,c,d,e,f'.split(',')) + '' === 'b,c,d,e,f,g').get('success') === true);

var inputObj = { x: 1, y: 2 };
var getAll = atom(inputObj);
assert('atomInstance.get with no params returns all data', getAll.get() === inputObj);
...
setTimeout(function () {
	...
	a.once(arr, function () {
		logger('Time to set ' + num + ' properties: ' + (new Date() - start) + 'ms');
	});
	while (++i < num) {
		set('z' + i);
	}
