...
(function (undef) {
	...
	var ObjProto = Object.prototype,
	    ...
	    typeUndef = '' + undef,
	    root = (typeof window === 'undefined' ? 'undefined' : _typeof(window)) !== typeUndef ? window : global;
	...
	function isCriticalError(data) {
		return data instanceof EvalError || typeof InternalError !== 'undefined' && data instanceof InternalError || data instanceof RangeError || data instanceof ReferenceError || data instanceof SyntaxError || data instanceof TypeError || data instanceof URIError;
	}
	...
	// Property getter
	function _get(nucleus, keyOrList, func) {
	...
	function performSet(config, nucleus, key, value) {
		...
		if (!had || oldValue !== value || isObj && !inArray(objStack, value)) {
			...
			while (i--) {
				...
				if (missing) {
					if (hasOwn.call(missing, key)) {
						delete missing[key];
						if (isEmpty(missing)) {
							listener.cb.apply({}, _get(nucleus, keys).values);
							listener.calls--;
						}
					}
			...
			if (config.persistenceLib) {
				...
				config.persistenceLib.set(config.modelName, Object.assign({}, nucleus.props, { lastStorageUpdateTimestamp: Date.now() }));
			}
		}
	}

	// Property setter
	function _set(config, nucleus, key, value, keyOrMap) {
		...
		if (config.validation && config.validation[key]) {
			...
			validationError = Object.keys(validation).find(function (validationKey) {
				...
				} else if (typeof validationResult.then === 'function' && typeof validationResult.catch === 'function') {
	...
	function _provide(config, nucleus, key, provider) {
		provider(preventMultiCall(function (result) {
			_set(config, nucleus, key, result);
		}));
	}
	...
	function atom() {
		...
		function getParsedValue(key, val) {
			return config.parsers && config.parsers[key] ? config.parsers[key](val) : val;
		}
		...
		function doNext() {
			if (q) {
				q.pending = q.next = !q.next && q.length ? q.shift() : q.next;
		...
		var me = {
			...
			clear: function clear() {
				...
				me.keys().forEach(function (key) {
					...
					promises.push(me.set(key, undefined, { validate: false }).then(function () {
						delete props[key];
					}));
				});
			...
			each: function each(keyOrList, func) {
			...
			entangle: function entangle(otherAtom, keyOrListOrMap) {
				var isList = isArray(keyOrListOrMap),
			...
			get: function get(keyOrList, func) {
				if (arguments.length === 0) {
					return props;
				}
				var result = _get(nucleus, keyOrList, func);
				if (!func) {
			...
			// values.
			need: function need(keyOrList, func) {
				var key,
				...
				for (var i = keys.length; --i >= 0;) {
					...
					if (!hasOwn.call(props, key) && provider) {
						_provide(config, nucleus, key, provider);
			...
			next: function next(keyOrList, func) {
				listeners.unshift({
					keys: toArray(keyOrList),
					cb: func,
					calls: 1
			...
			off: function off(keyOrList, func) {
				// alias: `unbind`
			...
			// than once.
			once: function once(keyOrList, func) {
				var keys = toArray(keyOrList),
				    results = _get(nucleus, keys),
			...
			provide: function provide(key, func) {
				if (needs[key]) {
					_provide(config, nucleus, key, func);
			...
			set: function set(keyOrMap, valueOrSetConfig, setConfig) {
				...
				if (isMap) {
					...
					return new Promise(function (resolve, reject) {
						...
						keys.forEach(function (key) {
							results[key] = _set(localConfig, nucleus, key, keyOrMap[key], keyOrMap);
				...
				return _set(localConfig, nucleus, keyOrMap, getParsedValue(keyOrMap, valueOrSetConfig));
			}
		};
		me.bind = me.on;
		me.unbind = me.off;
	...
	if ((typeof module === 'undefined' ? 'undefined' : _typeof(module)) !== typeUndef && module.exports) {
		module.exports = atom;
	} else {
		root.atom = atom;
	}
